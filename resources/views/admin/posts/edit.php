    ...
    <div class="bg-white rounded-lg shadow-md p-6">
        <form method="POST" action="/admin/posts/<?= $post['id'] ?>" enctype="multipart/form-data">
            ...
            <div class="mb-4">
                ...
                <input type="text" name="title" id="title" class="w-full px-3 py-2 border border-gray-300 rounded-md focus:outline-none focus:ring-2 focus:ring-blue-500" value="<?= htmlspecialchars($post['title']) ?>" required>
            </div>
            ...
            <div class="mb-4">
                ...
                <?php if (!empty($post['featured_image'])): ?>
                    <div class="mb-2">
                        <img src="<?= htmlspecialchars($post['featured_image']) ?>" alt="特色图片" class="w-32 h-32 object-cover rounded">
                    </div>
                <?php endif; ?>
            ...
            <div class="mb-4">
                ...
                <textarea name="excerpt" id="excerpt" rows="3" class="w-full px-3 py-2 border border-gray-300 rounded-md focus:outline-none focus:ring-2 focus:ring-blue-500"><?= htmlspecialchars($post['excerpt']) ?></textarea>
            </div>
            
            <div class="mb-4">
                <label for="content" class="block text-gray-700 text-sm font-bold mb-2">内容</label>
                <textarea name="content" id="content" rows="10" class="w-full px-3 py-2 border border-gray-300 rounded-md focus:outline-none focus:ring-2 focus:ring-blue-500" required><?= htmlspecialchars($post['content']) ?></textarea>
            </div>
            ...
            <div class="mb-4">
                ...
                <select name="category_id" id="category_id" class="w-full px-3 py-2 border border-gray-300 rounded-md focus:outline-none focus:ring-2 focus:ring-blue-500">
                    ...
                    <option value="<?= $category['id'] ?>" <?= ($post['category_id'] == $category['id']) ? 'selected' : '' ?>><?= htmlspecialchars($category['name']) ?></option>
                    <?php endforeach; ?>
                </select>
            </div>
            ...
            <div class="mb-6">
                ...
                <div class="flex flex-wrap gap-2">
                    ...
                    <label class="inline-flex items-center">
                        <input type="checkbox" name="tag_ids[]" value="<?= $tag['id'] ?>" class="rounded border-gray-300 text-blue-600 shadow-sm focus:border-blue-300 focus:ring focus:ring-blue-200 focus:ring-opacity-50" <?= (in_array($tag['id'], [])) ? 'checked' : '' ?>>
